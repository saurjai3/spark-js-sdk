...
describe(`plugin-search`, () => {
  ...
  describe(`#search`, () => {
    ...
    before(() => testUsers.create({count: 3})
      .then((users) => {
        ...
        return Promise.all([
          ...
          mccoy,
          spock
        ].map((user) => user.spark.mercury.connect()));
      }));

    let mccoyConversation, spockConversation;

    before(() => spockConversation || spock.spark.conversation.create({
    ...
    before(() => mccoyConversation || mccoy.spark.conversation.create({
    ...
    before(() => populateConversation(spockConversation)
      .then(() => {
    ...
    before(() => populateConversation(mccoyConversation)
      .then(() => {
